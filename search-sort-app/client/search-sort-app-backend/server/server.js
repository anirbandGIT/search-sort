...
var db = mongo.connect("mongodb://localhost:27017/car", {useNewUrlParser: true}, function (err, response) {
  // the database name is MyCar database
    if (err) {
        console.error(err);
    }
    else {
...
app.use(function (req, res, next) {
    ...
    res.setHeader('Access-Control-Allow-Methods', 'GET,POST'); //GET,POST,PUT,PATCH,DELETE
...
app.get('/api/v1/cars/:id', async function (req, res) {
  try {
    var task = await model.find({});

    if(req.params.id.startsWith('search=')) {
      req.params.id = req.params.id.replace("search=","");
    task = await task.filter(element => {
    return element.carName.toLowerCase().includes(req.params.id.toLowerCase());
    });
    }

    else if(req.params.id.startsWith('sort=')) {
      req.params.id = req.params.id.replace("sort=","");
      if(req.params.id === 'priceAscending') {
        await task.sort('carPrice', 1);
      }
      else if(req.params.id === 'priceDescending') {
        await task.sort('carPrice', -1);
      } else {
